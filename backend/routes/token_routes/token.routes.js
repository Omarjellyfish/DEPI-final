...
function TokenRouteFunc(userModel, adminModel,TokenRepos,TokenController
    ,tokenModel,
    BasicAuthMiddleware,adminRepos,userRepos
    ,CustomePasetoMiddleWare,CheckPermission){
    const route=Router();
    route.delete("/logout",(req,res,next)=>{
        console.log("jjuuuytyr443322")
        try{
            CustomePasetoMiddleWare(req,res,next,TokenController,userModel,adminModel,TokenRepos,tokenModel)
        }
        catch(err){
    ...
    ,async(req,res,next)=>{
       ...
       let  result=await new TokenController(userModel, adminModel,TokenRepos,tokenModel,next).deleteUserToken(req.userId)
        res.send({"res":"done"})
        return;
    })
